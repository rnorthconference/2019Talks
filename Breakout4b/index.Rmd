...
On average, there are **1.47** honeybee permits per neighborhood. 
...
Unfortunately for us, "neighborhoods" and "neighbors" have two very distinct meanings in this particular analysis. In spatial data analysis, we think of "neighbors" as how we define whether or not two objects are spatially related. 
...
```{r echo = FALSE, fig.cap="Source: Manuel Gimond"}
...
__Note: The Monte-Carlo approach to significance__

In order to test the significance of our correlation, we *randomly assign* attribute values to the polygons, calculate Moran's I, and then repeat for a given number of permutations. Then we compare our observed attribute values to this sampling distribution of Moran’s I values calculated **under the null hypothesis (complete spatial randomness)**.
